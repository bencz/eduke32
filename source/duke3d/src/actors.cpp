...
ACTOR_STATIC void G_MoveFX(void)
{
    ...
    while (spriteNum >= 0)
    {
        ...
        case RESPAWN__:
            ...
            // "Damn I'm Good" delayed respawn marker: xvel holds the tics left
            // before the killed enemy comes back (see G_DigRespawnKilledEnemy).
            // When it elapses, fire the teleporter star and hand off to the
            // native finishing countdown above (extra 54 -> 66).
            else if (pSprite->xvel > 0)
            {
                if (--sprite[spriteNum].xvel <= 0)
                {
#ifndef EDUKE32_STANDALONE
                    if (!FURY)
                    {
                        int const star = A_Spawn(spriteNum, TRANSPORTERSTAR);
                        sprite[star].z -= ZOFFSET5;
                    }
#endif
                    sprite[spriteNum].extra = 66-12;
                }
            }
            break;
...
static bool A_IsRecyclableDebris(int const picnum)
{
    // Purely cosmetic sprites generated by combat: blood, gibs, shells, scrap,
    // glass, smoke, etc. Always safe to drop.
    return picnum == BLOOD        || picnum == BLOODPOOL
        || picnum == GLASSPIECES  || picnum == BULLETHOLE
        || picnum == SMALLSMOKE   || picnum == WATERBUBBLE
        || picnum == FECES        || picnum == BURNEDCORPSE
        || (picnum >= JIBS1  && picnum <= JIBS6)
        || (picnum >= SCRAP6 && picnum <= SCRAP5 + 3);
}

static bool A_IsRecyclablePickup(int const picnum)
{
    // Weapon and ammo pickups (incl. the ones monsters drop on death). With the
    // "Damn I'm Good" flood these pile up forever and eat the sprite array, so
    // we let them be reclaimed. Shells/casings count too. Keys, health and
    // inventory are deliberately left alone so progression can't break.
    return (picnum >= FIRSTGUNSPRITE && picnum <= GROWSPRITEICON)  // weapons
        || (picnum >= FREEZEAMMO     && picnum <= SHOTGUNAMMO)     // ammo
        || picnum == SHELL || picnum == SHOTGUNSHELL;
}

// When the sprite array runs low on free slots (because the "Damn I'm Good"
// monster flood plus all the resulting corpses, dropped weapons and combat
// debris fill it up), recycle the least important sprites so new monsters /
// respawns / projectiles / effects can keep spawning. Without this the array
// saturates and monsters simply stop appearing. This is a no-op until the array
// gets genuinely crowded, so it stays essentially free on a normal tic.
static void G_RecycleDebrisSprites(void)
{
    // Start recycling once fewer than this many free slots remain...
    int const lowWaterMark  = 2048;
    // ...and keep going until we have clawed back at least this much headroom.
    int const highWaterMark = 4096;

    if (MAXSPRITES - Numsprites > lowWaterMark)
        return;

    // Pass 1: cosmetic debris and dropped weapon/ammo pickups (STAT_MISC).
    {
        int spriteNum, nextSprite;
        for (SPRITES_OF_STAT_SAFE(STAT_MISC, spriteNum, nextSprite))
        {
            int const picnum = sprite[spriteNum].picnum;

            if (A_IsRecyclableDebris(picnum) || A_IsRecyclablePickup(picnum))
            {
                A_DeleteSprite(spriteNum);

                if (MAXSPRITES - Numsprites >= highWaterMark)
                    return;
            }
        }
    }

    // Pass 2: dropped weapon/ammo pickups living in STAT_ACTOR. (Monster corpses
    // are bounded separately via the engine deletion queue, see G_MoveActors.)
    {
        int spriteNum, nextSprite;
        for (SPRITES_OF_STAT_SAFE(STAT_ACTOR, spriteNum, nextSprite))
        {
            if (A_IsRecyclablePickup(sprite[spriteNum].picnum))
            {
                A_DeleteSprite(spriteNum);
